...
###############################################################################
#
# Este script es idéntico al de la app, pero para el portal
#
...
# 5. EC2_INSTANCE_NAME: Nombre para la instancia (ej: shopping-portal)
#
###############################################################################
...
EC2_INSTANCE_NAME="shopping-flutter"  # Nombre para la instancia (compartida con app)
...
ssh -i "$KEY_FILE" -o StrictHostKeyChecking=no ec2-user@${PUBLIC_IP} << ENDSSH | tee -a "$HISTORY_FILE"
    ...
    if [ ! -d "\$REPO_NAME/shopping_exercise_portal" ]; then
        echo "  ❌ ERROR: Directorio shopping_exercise_portal no encontrado en \$REPO_NAME"
    ...
    echo "  📁 Ubicación: /home/ec2-user/\$REPO_NAME/shopping_exercise_portal"
ENDSSH
...
ssh -i "$KEY_FILE" -o StrictHostKeyChecking=no ec2-user@${PUBLIC_IP} << ENDSSH | tee -a "$HISTORY_FILE"
    ...
    echo "  → Actualizando configuración de nginx (agregando portal Flutter)..."
    
    # Verificar que la app ya esté compilada
    if [ ! -d "/home/ec2-user/\$REPO_NAME/shopping_exercise_app/build/web" ]; then
        echo "  ⚠️  La app aún no está compilada. Ejecuta primero deploy_app_flutter_ec2.sh"
    fi
...
echo ""
echo "🌐 🌐 🌐 TODO ESTÁ DISPONIBLE EN: 🌐 🌐 🌐"
echo ""
echo "   👉 Backend API: http://${PUBLIC_IP}/api 👈"
echo "   👉 App Flutter: http://${PUBLIC_IP}/app 👈"
echo "   👉 Portal Flutter: http://${PUBLIC_IP}/portal 👈"
echo ""
echo "   ✅ Todo está en la misma instancia EC2 (shopping-app)"
echo "   ✅ Backend protegido detrás de nginx (no expuesto directamente)"
echo "   ✅ Una sola IP pública para todo"
echo ""
